...
function install_dependencies() {
    ...
    for pkg in "${packages[@]}"; do
        if ! command -v $pkg &> /dev/null; then
            ...
            apt update -y && apt install -y $pkg || { echo "Failed to install $pkg. Exiting."; exit 1; }
        else
...
VERSION=2024091801
BASE_URL=https://kali.download/nethunter-images/kali-2024.3/rootfs/
USERNAME=kali
...
# Kaam No. 1: Update termux.properties
echo "[*] Performing Kaam No. 1: Updating termux.properties..."
TERMUX_PROPERTIES_FILE="$HOME/.termux/termux.properties"

if [ ! -f "$TERMUX_PROPERTIES_FILE" ]; then
    echo "Error: File $TERMUX_PROPERTIES_FILE does not exist. Please ensure Termux is properly set up."
else
    if grep -qE "^\s*#\s*allow-external-apps\s*=" "$TERMUX_PROPERTIES_FILE"; then
        sed -i -E 's/^\s*#\s*(allow-external-apps\s*=.*)/\1/' "$TERMUX_PROPERTIES_FILE"
        echo "The line 'allow-external-apps=true' has been uncommented in $TERMUX_PROPERTIES_FILE."
    else
        echo "Line for 'allow-external-apps' not found in $TERMUX_PROPERTIES_FILE. No changes made."
    fi
fi
